...
|---|---|---|---|---|---|---|---|---|---|
| P0 | STORY-081 | - | Implement role-based access control (RBAC) | READY_FOR_PO | po | `backend/identity-access`, incremental `backend/platform-core` authorization wiring, Flyway migration(s), backend RBAC API/tests | No | 2026-05-24 23:40 local | `po` should validate the QA-approved backend-only RBAC foundation against the story intent and decide acceptance based on the representative protected-route evidence and migration/audit contract. |
